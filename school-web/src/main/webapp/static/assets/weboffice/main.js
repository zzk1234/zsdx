...
function editDoc(url) {
	try{
		var webObj=document.getElementById("WebOffice1");
		//var doctype=document.getElementById("doctype").value;
		webObj.LoadOriginalFile(url, "doc");
	}catch(e){
		alert("异常\r\nError:"+e+"\r\nError Code:"+e.number+"\r\nError Des:"+e.description);
	}
}
...
function SaveUploadDoc() {
	try{
		...
		var url=sessionStorage.webOfficeSaveUrl;
        //console.log(url);
...
*
/****************************************************/
function SaveDoc(postUrl) {
	try{
		var webObj=document.getElementById("WebOffice1");
		var returnValue;
		
		webObj.HttpInit(); // 初始化Http 引擎  
    	webObj.HttpAddPostCurrFile("file","正文");    
...
function GetNowDateTime()
 {
    ...
    var NowDateTime = nowTime.getFullYear() + "年" + month + "月" + 
                        nowTime.getDate() + "日 " + nowTime.getHours() + ":" +
                        nowTime.getMinutes() + ":" +
                        nowTime.getSeconds() + "." +
