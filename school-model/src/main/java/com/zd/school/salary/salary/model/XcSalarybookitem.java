...
/**
 * 
 * ClassName: XcSalarybookitem Function: TODO ADD FUNCTION. Reason: TODO ADD
 * REASON(可选). Description: 工资信息表(XC_T_SALARYBOOKITEM)实体类. date: 2016-12-05
 *
 * @author luoyibo 创建文件
 * @version 0.1
 * @since JDK 1.8
...
@Entity
@Table(name = "XC_T_SALARYBOOKITEM")
@AttributeOverride(name = "uuid", column = @Column(name = "SALARYBOOKITEM_ID", length = 36, nullable = false))
public class XcSalarybookitem extends BaseEntity implements Serializable {
	...
	@Formula("(SELECT a.SALARY_NAME FROM XC_T_SALARYBOOK a WHERE a.SALARYBOOK_ID=SALARYBOOK_ID)")	
	...
	@Formula("(SELECT a.JXBOOK_ID FROM XC_T_SALARYBOOK a WHERE a.SALARYBOOK_ID=SALARYBOOK_ID)")	
	@FieldInfo(name = "绩效工资台账Id")
